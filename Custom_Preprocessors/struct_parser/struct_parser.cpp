...
std::vector<std::string> header_make_emplace_backs(std::vector<std::string> struct_token, std::vector<std::string> element_token, std::unordered_map<std::size_t, std::size_t> link, std::string& namespace_name) {
  ...
  //res.push_back(struct_token[i] + token_link)
  res.push_back("// File auto generated by scanning \"components.hpp\" by struct_parser.exe, from Custom_Preproceccors // header file");
  res.push_back("#pragma once");
  res.push_back("#include <memory>");
  res.push_back("#include \"memory.hpp\"");
  res.push_back("#include \"components.hpp\"");
  res.push_back("");
  ...
  res.push_back("");
  res.push_back("template <typename component>");
  res.push_back("void create_component(std::unique_ptr<component>& pointer);"); // base function declaration

  for (int i = 0; i < struct_token.size(); i++) { // overloads/specialization declaration
    
    res.push_back(" ");
    res.push_back("template <>");
    res.push_back(std::string() + "void create_component<" + namespace_name + "::" + struct_token[i] + ">(" + "std::unique_ptr<" + namespace_name + "::" + struct_token[i] + ">&" + " pointer);");
...
std::vector<std::string> header_make_deletion(std::vector<std::string> struct_token, std::vector<std::string> element_token, std::unordered_map<std::size_t, std::size_t> link, std::string& namespace_name) {
  std::vector<std::string> res;
  std::string previous_struct = "";
  ...
  res.push_back("");
  res.push_back("template <typename component>");
  res.push_back("void delete_component(std::unique_ptr<component>& pointer, std::size_t& index);");

  for (int i = 0; i < struct_token.size(); i++) {
   
    res.push_back(" ");
    res.push_back("template <>");
    res.push_back(std::string() + "void delete_component<" + namespace_name + "::" + struct_token[i] + ">(" + "std::unique_ptr<" + namespace_name + "::" + struct_token[i] + ">&" + " pointer, std::size_t& index);");
